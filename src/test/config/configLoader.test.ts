...
suite('stripJsonComments utility', () => {
  ...
  setup(async function () {
    this.timeout(30000);
    tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'repomix-test-'));

    // Copy existing official config file from project root as a recent sample
    const rootConfigPath = path.join(__dirname, '../../../repomix.config.json');
    await fsp.copyFile(rootConfigPath, path.join(tempDir, 'repomix.config.json'));
  });
